...
dbDisconnect(con)
```
jeśli procent_popularności < 80 dla wycieczki x
Wycieczka x nie cieszy się satysfakcjonującym poziomem popularności, po zakończeniu każdej wycieczki tej kategori, powiniśmy poprosić klientów o wyrażenie swoich opinii i dostować się do ich głosów.
...
dbDisconnect(con)
```
Trzy najbardziej dochodowe wycieczki to !!wymienić te wycieczki!!. W następnym roku naszej działalności powiniśmy zainwestować w infrastrukturę umożliwiającą zwiększenie liczby tych wycieczek, aby zmaksymalizować nasz zysk.

## Udział usług dodatkowych w zyskach
Wykres przedstawia analizę zysków wygenerowanych przez różne rodzaje usług dodatkowych w stosunku do kosztów poniesionych na rzecz kontrahentów. Usługi dodatkowe stanowią istotny element oferty, który nie tylko zwiększa atrakcyjność wycieczek, ale również przyczynia się do wzrostu dochodów. Dzięki szczegółowemu zestawieniu można zidentyfikować najbardziej dochodowe usługi, a także te, które wymagają optymalizacji lub większej promocji.
...
```{r Proporcja klientek do klientow}
what <- readRenviron('../.env')
pass <- Sys.getenv("CONNECTION_PASSWORD")
...
dbDisconnect(con)
```
jeśli k>m i (k-m)/k > 0.1 dla wycieczki x, to:

Wycieczka x cieszy się większą popularnością wśród kobiet. Aby jeszcze bardziej zwiększyć jej atrakcyjność, warto rozważyć przeprowadzenie kampanii marketingowej skierowanej specjalnie do tej grupy docelowej, co może przyczynić się do dalszego wzrostu jej popularności wśród kobiet.

jeśli m>k i (m-k)/m>0.1 dla wycieczki x, to:

Wycieczka x jest bardziej popularna wśród mężczyzn. W związku z tym, aby jeszcze skuteczniej przyciągnąć tę grupę, powinniśmy zwiększyć liczbę reklam wycieczki x skierowanych bezpośrednio do mężczyzn, co może przyczynić się do dalszego wzrostu liczby uczestników męskiej części tej wycieczki.
else:

Nie występują wyraźne preferencje związane z płcią w wyborze wycieczki x. Oferta powinna być skierowana do obu grup w równym stopniu, uwzględniając szeroką gamę działań marketingowych, które nie będą faworyzować jednej płci.
