...
# reorder(df$Rodzaj_wycieczki, -df$Procent_popularności)
...
# ggplot(data = df, mapping = aes(x = reorder(nazwa, -Łączna_Kwota),
#                                 y = Łączna_Kwota,
#                                 fill = nazwa)) +
#   geom_bar(stat = 'identity', 
#            width = 0.55) +
#   geom_text(label = paste0(df$Łączna_Kwota, "zł"),
#             check_overlap = TRUE,
#             nudge_y = 5,
#             vjust = -0.25) +
#   scale_y_continuous(breaks = seq(from=0, to=1.1*max(df$Łączna_Kwota), length.out = 5)) +
#   xlab("Kontrahent") +
#   ylab("Poniesione koszty") +
#   ggtitle("Koszty na rzecz kontrahentów") +
#   theme(axis.text.x = element_blank(),
#         axis.ticks.x = element_blank(),
#         axis.title = element_blank())
```
...
# df$Miesiąc <- lapply(df$Miesiąc, FUN = function(x) substring(x, 6))
# # levels(df$Miesiąc)
# df$Miesiąc

# as.roman(unlist(lapply(df$Miesiąc, FUN = function(x) substring(x, 6))))
